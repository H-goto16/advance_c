...
int main(void) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (p->taijuu > fitness[max_weight_index].taijuu) {
      max_weight_index = i;
    }

    if (p->shinchou > fitness[max_height_index].shinchou) {
      max_height_index = i;
    }
  ...
  printf("平均年齢 : %f\n平均身長 : %f\n平均体重 : %.3f\n体重の最大値とその会員番号 : %f, %d\n身長の最大値とその会員番号 : %.3f, %d\n",
         avg_age, avg_height, avg_weight, fitness[max_weight_index].taijuu, fitness[max_weight_index].bangou, fitness[max_height_index].shinchou, fitness[max_height_index].bangou);
